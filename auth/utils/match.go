...
const (
	// 标签的正则匹配, 合法的字符有 0-9, A-Z, a-z, 汉字
	labelPattern = `^[a-z0-9A-Z\p{Han}]*$`
	...
	// 文件名称的正则匹配, 合法的字符有 0-9, A-Z, a-z, _, - 汉字
	filenamePattern = `^[a-z0-9A-Z\p{Han}-_]*$`
...
func IsLabelString(str string) bool {
	if len(str) <= 0 {
		return false
	}
	return labelRegexp.MatchString(str)
}
...
func IsFilenameString(str string) bool {
	if len(str) == 0 {
		return false
	}
	return filenameRegexp.MatchString(str)
}
